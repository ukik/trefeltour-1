...
<template>
    <div class="mb-2 mt-3 p-0 col ml-3 pr-2 row">
        ...
        <label class="badaso-text__label col-12 p-1">Pilih Vendor</label>
        ...
        <vue-typeahead-bootstrap disabled ref="typeahead" class="col p-0" :class="[ $route?.name == 'CrudGeneratedEdit' ? 'mr-4' : '']"  v-model="query" :ieCloseFix="false" :data="users"
            :serializer="item => { return `Vendor UUID (${item.uuid})` }"
            @hit="selecteduser = $event" placeholder="Pilih Vendor" @input="lookupUser" required>
        ...
        <stack-modal
            ...
            <slot name="modal-header">
                <div class="modal-header">
                    <h3 class="modal-title">
                        {{  type == 'detail' ? 'Detail' : 'Pilih' }} Vendor
                    </h3>
            ...
            <shared-read-user :response="{
                data: selecteduser
            }" v-if="type=='detail'" slug="travel-stores"></shared-read-user>

            <shared-table-modal v-if="type=='select'" @onBubbleEvent="onBubbleEvent" slug="travel-stores" />
...
export default {
    ...
    async mounted() { this.$openLoader();
        ...
        if(this.$route.params?.id) {
            axios
                .get(`/api/typehead/travel/dialog_prices_travel_stores?id=` + this.$route.params?.id, {
                    headers: {
                ...
                .then(response => {
                    ...
                    this.$refs.typeahead.inputValue = `Vendor UUID (${item.uuid})` //`Profile UUID (${item.uuid}) - Nama (${item?.badasoUser.name}) - Username (${item?.badasoUser.username}) - Email (${item?.badasoUser.email}) - Telpon (${item?.badasoUser.phone})`
    ...
    methods: {
        onBubbleEvent(response) {
            ...
            const item = response
            this.$refs.typeahead.inputValue = `Vendor UUID (${item.uuid})`
        ...
        lookupUser: debounce(function () {
            ...
            axios
                .get('/api/typehead/travel/dialog_prices_travel_stores?keyword=' + this.query, {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem('token')}`
                    }
                })
