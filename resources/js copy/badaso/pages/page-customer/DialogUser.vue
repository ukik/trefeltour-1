<template>
  <div class="mb-2 mt-3 p-0 col ml-3 pr-2 row">
    ...
    <vue-typeahead-bootstrap
      ...
      :serializer="
        (item) => {
          return `Nama (${item.name}) Email (${item.email}) Telp (${item.phone})`;
        }
      "
    ...
    <div
      v-if="$route?.name == 'CrudGeneratedAdd' && userRole !== 'admin-lodge'"
      @click="onHapus"
...
export default {
  ...
  methods: {
    onGet(val) {
      axios
        .get(`/api/typehead/lodge/user?id=` + val, {
          headers: {
    ...
    lookupUser: debounce(function () {
      ...
      return;
      axios
        .get("/api/typehead/lodge/user?keyword=" + this.query, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
        })
