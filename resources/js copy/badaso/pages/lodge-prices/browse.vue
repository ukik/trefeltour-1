    ...
    >
      ...
      <div class="py-4">
        ...
        <div class="row">
          ...
          <span class="col-auto">{{ selectedQuantity }} hari</span>
        </div>
        <div class="row">
      ...
      <div slot="modal-footer">
        ...
        <vs-row>
          <vs-col>
            ...
            <div v-if="false" class="full-width text-center pb-2">
              <vs-button
                @click="is_calender_pilih = true"
                color="primary"
                :type="is_calender_pilih ? 'filled' : 'border'"
                >Pilih Tanggal Check-In</vs-button
              >
              <vs-button
                @click="is_calender_pilih = false"
                color="success"
                :type="!is_calender_pilih ? 'filled' : 'border'"
                >Lihat Sudah Booked</vs-button
              >
            </div>
            <div v-if="false" class="full-width text-center pb-2">
        ...
        <div class="modal-header">
          ...
          <vs-button
            :disabled="!selectedCustomer || selectedQuantity <= 0"
            @click="onAddToCart"
          >
                          ...
                          <vs-dropdown-menu>
                            ...
                            <badaso-dropdown-item
                              :to="{
                                name: 'CrudGeneratedRead',
                                params: {
                                  id: data[index].tourismVenue?.tourismService.id,
                            ...
                            <badaso-dropdown-item
                              v-for="(item1, index1) in data[index].tourismVenue
                                ?.tourismFacilities"
                              :key="1 + index1"
                              :to="{
                                name: 'CrudGeneratedRead',
  ...
  methods: {
    ...
    async onAddToCart() {
      ...
      if (this.selectedQuantity <= 0) {
        this.$vs.notify({
          title: this.$t("alert.danger"),
          text: "Durasi tidak boleh 0 hari",
          color: "danger",
        });
        return;
      }
      ...
      this.$openLoader();
      await axios
        .post("/api/typehead/lodge/add_to_cart", bodyFormData, {
          headers: {
