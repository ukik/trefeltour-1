...
<template>
    <div class="mb-2 mt-3 p-0 col ml-3 pr-2 row">
        ...
        <label class="badaso-text__label col-12 p-1">User Promotor</label>
        ...
        <vue-typeahead-bootstrap disabled ref="typeahead" class="col p-0" :class="[ $route?.name == 'CrudGeneratedEdit' ? 'mr-4' : '']"  v-model="query" :ieCloseFix="false" :data="users"
            ...
            @hit="selecteduser = $event" placeholder="Pilih User Promotor" @input="lookupUser" required>
...
export default {
    ...
    methods: {
        onGet(val) {
            axios
                .get(`/api/typehead/offers/user?id=` + val, {
                    headers: {
        ...
        lookupUser: debounce(function () {
            ...
            axios
                .get('/api/typehead/offers/user?keyword=' + this.query, {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem('token')}`
                    }
                })
