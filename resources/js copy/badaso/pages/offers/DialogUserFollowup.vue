...
<template>
    <div class="mb-2 mt-3 p-0 col ml-3 pr-2 row">
        ...
        <label class="badaso-text__label col-12 p-1">Admin Follow Up</label>
        ...
        <vue-typeahead-bootstrap disabled ref="typeahead" class="col p-0" :class="[ $route?.name == 'CrudGeneratedEdit' ? 'mr-4' : '']"  v-model="query" :ieCloseFix="false" :data="users"
            ...
            @hit="selecteduser = $event" placeholder="Pilih Admin Follow Up" @input="lookupUser" required>
        </vue-typeahead-bootstrap>
        <div v-if="$route?.name == 'CrudGeneratedAdd' && userRole !== 'admin-offers'" @click="onHapus" class="btn btn-primary col-auto mr-4">
            Hapus
        </div>
...
export default {
    ...
    methods: {
        onGet(val) {
            axios
                .get(`/api/typehead/offers/user_followup?id=` + val, {
                    headers: {
        ...
        lookupUser: debounce(function () {
            ...
            axios
                .get('/api/typehead/offers/user_followup?keyword=' + this.query, {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem('token')}`
                    }
                })
