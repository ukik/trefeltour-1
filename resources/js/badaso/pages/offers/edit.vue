            ...
            <vs-row>
              ...
              >
                <template v-if="dataRow.edit && dataRow.type !== 'hidden'">
                  ...
                  ></badaso-password>
                  <badaso-textarea
                    v-if="dataRow.type == 'textarea' && dataRow.field !== 'catatan_followup'"
                    :label="dataRow.displayName"
                    :placeholder="dataRow.displayName"
                    v-model="dataRow.value"
                    size="12"
                  ...
                  <badaso-textarea
                    v-if="dataRow.type == 'textarea' && dataRow.field == 'catatan_followup' && $store.getters['custom/isAdmin']"
                    :label="dataRow.displayName"
                    :placeholder="dataRow.displayName"
                    v-model="dataRow.value"
                    size="12"
                  ...
                  <!-- <badaso-textarea
                    v-if="dataRow.type == 'textarea_readonly'"
                  ...
                  ></badaso-checkbox>
                  <badaso-select
                    v-if="dataRow.type == 'select' && dataRow.field !== 'is_followup'"
                    :label="dataRow.displayName"
                    :placeholder="dataRow.displayName"
                    v-model="dataRow.value"
                    size="12"
                  ...
                  <badaso-select
                    v-else-if="dataRow.type == 'select' && dataRow.field == 'is_followup' && $store.getters['custom/isAdmin']"
                    :label="dataRow.displayName"
                    :placeholder="dataRow.displayName"
                    v-model="dataRow.value"
                    size="12"
...
import DialogUserFollowup from './DialogUserFollowup.vue'

export default {
  name: "CrudGeneratedEdit",
  components: {
    DialogUser,DialogUserFollowup
  },
  data: () => ({
    isValid: true,
    errors: {},
        ...
        temp.forEach(el => {
            for (const key in this.record) {
                if (Object.hasOwnProperty.call(this.record, key)) {
                    ...
                    if(el.field == 'end_date' && key == 'endDate') {
                        el.value = new Date(this.record[key]);
                    }

                    if(el.field == 'is_followup' && key == 'isFollowup') {
                        if(!this.$store.getters['custom/isAdmin']) el.type = 'textarea_readonly';
                    }

                    if(el.field == 'catatan_followup' && key == 'catatanFollowup') {
                        if(!this.$store.getters['custom/isAdmin']) el.type = 'select_readonly';
                    }

                    if(el.field == 'file_prosposal' && key == 'fileProsposal') {
                        el.value = (this.record[key])?.toString();
                    }

                    if(el.field == 'image_prosposal' && key == 'imageProsposal') {
                        el.value = (this.record[key])?.toString();
                    }
