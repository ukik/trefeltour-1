...
<template>
    <div class="mb-2 mt-3 p-0 col ml-3 pr-2 row">
        ...
        <label class="badaso-text__label col-12 p-1">Pilih Skill</label>
        ...
        <vue-typeahead-bootstrap disabled ref="typeahead" class="col p-0" :class="[ $route?.name == 'CrudGeneratedEdit' ? 'mr-4' : '']"  v-model="query" :ieCloseFix="false" :data="users"
            :serializer="item => { return `Skill UUID (${item.uuid}) - Nama (${item?.talentProfile?.badasoUser.name}) - Username (${item?.talentProfile?.badasoUser.username}) - Email (${item?.talentProfile?.badasoUser.email}) - Telpon (${item?.talentProfile?.badasoUser.phone})` }"
            @hit="selecteduser = $event" placeholder="Pilih Skill" @input="lookupUser" required>
...
export default {
    ...
    async mounted() { this.$openLoader();
        ...
        if(this.$route.params?.id) {
            axios
                .get(`/api/typehead/talent/dialog_profile_talent_skills?id=` + this.$route.params?.id, {
                    headers: {
                ...
                .then(response => {
                    ...
                    this.$refs.typeahead.inputValue = `Skill UUID (${item.uuid}) - Nama (${item?.talentProfile?.badasoUser.name}) - Username (${item?.talentProfile?.badasoUser.username}) - Email (${item?.talentProfile?.badasoUser.email}) - Telpon (${item?.talentProfile?.badasoUser.phone})` //`Profile UUID (${item.uuid}) - Nama (${item?.badasoUser.name}) - Username (${item?.badasoUser.username}) - Email (${item?.badasoUser.email}) - Telpon (${item?.badasoUser.phone})`
    ...
    methods: {
        onBubbleEvent(response) {
            ...
            const item = response
            this.$refs.typeahead.inputValue = `Skill UUID (${item.uuid}) - Nama (${item?.talentProfile?.badasoUser.name}) - Username (${item?.talentProfile?.badasoUser.username}) - Email (${item?.talentProfile?.badasoUser.email}) - Telpon (${item?.talentProfile?.badasoUser.phone})`
        ...
        lookupUser: debounce(function () {
            ...
            axios
                .get('/api/typehead/talent/dialog_profile_talent_skills?keyword=' + this.query, {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem('token')}`
                    }
                })
