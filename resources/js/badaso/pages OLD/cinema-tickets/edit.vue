              ...
              >
                <template v-if="dataRow.edit && dataRow.type !== 'hidden'">
                  ...
                  <!-- <vs-input type="text" v-model="dataRow.value"></vs-input> -->
                    ...
                    <!-- ADDITIONAL -->
                    <badaso-text v-if="dataRow.field == 'code_ticket'" text_readonly :label="dataRow.displayName"
                  ...
                  <badaso-text
                    v-if="dataRow.type == 'text' && dataRow.field !== 'code_ticket'"
                  ...
                  ></badaso-date>
                  <badaso-datetime
                    v-if="dataRow.type == 'datetime'  && dataRow.field !== 'purchase_date'"
                    :label="dataRow.displayName"
                    :placeholder="dataRow.displayName"
                    v-model="dataRow.value"
                    size="12"
...
export default {
  ...
  methods: {
    onAlert(error, label = null) {
        console.log('onAlert '+label, error)
        this.$closeLoader();
        this.$vs.notify({
    ...
    submitForm() {
      ...
      // ADDITIONAL
      const vm = this
      let last_seat_id = vm.last_seat_id
        ...
        this.$api.badasoEntity.read({
            ...
            console.log('seat id', data.id, vm.last_seat_id)

            if (data.seatStatus == 'available' && data.id != vm.last_seat_id) {
                ...
                this.$api.badasoEntity.edit({
                    ...
                    if(vm.last_seat_id != response.data.id) {
                        // mengganti yang not available ke available
                        // jika seat berbeda dari sebelumnya
                        this.$api.badasoEntity.read({
                            slug: 'cinema-seats',
                            id: vm.last_seat_id
                        }).then((response) => {
                    ...
                    console.log('cinema-shows response 3', response)
    ...
    async getDetailEntity() {
      ...
      try {
        ...
        // ADDITIONAL
        console.log('this.dataType.dataRows', this.dataType.dataRows)
        for (const row of this.dataType.dataRows) {
            if(row.field == 'seat_id') {
                console.log(row.field, row.value)
                this.last_seat_id = row.value
                break
            }
        }
