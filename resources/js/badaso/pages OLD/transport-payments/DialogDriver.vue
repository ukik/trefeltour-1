...
<template>
    <div class="mb-2 mt-3 p-0 col ml-3 pr-2 row">
        ...
        <vue-typeahead-bootstrap disabled ref="typeahead" class="col p-0" :class="[ $route?.name == 'CrudGeneratedEdit' ? 'mr-0' : '']"  v-model="query" :ieCloseFix="false" :data="users"
        ...
        <stack-modal
            ...
            <shared-read-user :response="{
                data: selecteduser
            }" v-if="type=='detail'" slug="transport-drivers"></shared-read-user>

            <shared-table-modal v-if="type=='select'" @onBubbleEvent="onBubbleEvent" slug="transport-drivers" />
...
export default {
    ...
    async mounted() { this.$openLoader();
        ...
        if(this.$route.params?.id) {
            axios
                .get(`/api/typehead/transport/dialog_payment_transport_drivers?id=` + this.$route.params?.id, {
                    headers: {
    ...
    methods: {
        ...
        lookupUser: debounce(function () {
            ...
            axios
                .get('/api/typehead/transport/user?keyword=' + this.query, {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem('token')}`
                    }
                })
