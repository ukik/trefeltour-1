...
export default {
    ...
    async mounted() { this.$openLoader();
        ...
        if(this.$route.params?.id) {
            axios
                .get(`/api/typehead/tourism/dialog_venue_tourism_bookings?id=` + this.$route.params?.id, {
                    headers: {
    ...
    methods: {
        ...
        lookupUser: debounce(function () {
            ...
            axios
                .get('/api/typehead/tourism/dialog_venue_tourism_bookings?keyword=' + this.query, {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem('token')}`
                    }
                })
