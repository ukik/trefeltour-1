...
<template>
    <div class="mb-2 mt-3 p-0 col ml-3 pr-2 row">
        ...
        <!-- {{ userRole !== 'admin-souvenir' }} xxxxxxxxxxxxxx -->
        ...
        <div v-if="$route?.name == 'CrudGeneratedAdd' && userRole !== 'admin-souvenir'" @click="onHapus" class="btn btn-primary col-auto mr-4">
            Hapus
        </div>
        ...
        <stack-modal
            ...
            <shared-read-user :response="{
                data: selecteduser
            }" v-if="type=='detail'" slug="souvenir-payments"></shared-read-user>

            <shared-table-modal-payment-validation v-if="type=='select'" @onBubbleEvent="onBubbleEvent" slug="souvenir-payments" />
...
export default {
    ...
    async mounted() { this.$openLoader();
        ...
        if(this.$route.params?.id) {
            axios
                .get(`/api/typehead/souvenir/dialog_booking_souvenir_payments_validations?id=` + this.$route.params?.id, {
                    headers: {
    ...
    methods: {
        ...
        lookupUser: debounce(function () {
            ...
            axios
                .get('/api/typehead/souvenir/dialog_booking_souvenir_payments_validations?keyword=' + this.query, {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem('token')}`
                    }
                })
