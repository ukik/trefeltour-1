    ...
    <stack-modal class="d-flex justify-content-center"
            ...
            <div v-if="tipe == 'single'" class="py-4">
                ...
                <div class="row">
                      ...
                      <span class="col-auto">{{ selectedData?.transportPrice?.uuid }}</span>
                  </div>
                  <div class="row">
                      <span class="col">Nama Harga </span>
                      <span class="col-auto">{{ selectedData?.transportPrice?.name }}</span>
                  </div>
                  ...
                  <div class="row">
                      <span class="col">Harga per malam</span>
                      <span class="col-auto">{{ $rupiah(getTotalAmount(selectedData?.transportPrice)) }}</span>
                  </div>
                  <div class="row">
                      <span class="col">Total Harga</span>
                      <span class="col-auto">
                          {{ $rupiah(Math.round(getTotalAmount(selectedData?.transportPrice) * selectedData?.quantity)) }}
                      </span>
                  </div>
                  <div class="row">
                        <div class="col">
                            <!-- <div class="full-width text-center">Tanggal Check-In</div> -->
                            <div class="full-width text-center p-2">
                            ...
                            <CalenderCheckIn v-show="is_calender_pilih" @onBubbleEventUpdate="selectedData = $event" @onBubbleEvent="records = $event"
                                ...
                                :date_checkin="selectedData?.dateCheckin" />
            ...
            <div v-if="tipe == 'multi'" class="py-4" :class="[ selectedMulti.length >= 2 ? 'pr-2' : '' ]" style="
                ...
                <template v-for="(item, index) in selectedMulti" >
                    <div>
                        ...
                        <div class="row">
                            <span class="col">UUID </span>
                            <span class="col-auto">{{ item?.transportPrice?.uuid }}</span>
                        </div>
                        <div class="row">
                            <span class="col">Nama Harga </span>
                            <span class="col-auto">{{ item?.transportPrice?.name }}</span>
                        </div>

                        <!-- <div class="row">
                            <span class="col">Tempat </span>
                            <span class="col-auto">{{ item?.transportProfile?.name }}</span>
                        </div>
                        <div class="row">
                            <span class="col">Kamar</span>
                            <span class="col-auto">{{ item?.transportRoom?.name }}</span>
                        </div>
                        <div class="row">
                            <span class="col">Stok</span>
                            <span class="col-auto">{{ item?.transportRoom?.quota }} kamar</span>
                        </div> -->

                        <div class="row">
                            <span class="col">Rental </span>
                            <span class="col-auto">{{ item?.transportRental?.name }}</span>
                        </div>
                        <div class="row">
                            <span class="col">Unit </span>
                            <span class="col-auto">{{ item?.transportVehicle?.model }}</span>
                        </div>
                        <div class="row">
                            <span class="col">Durasi</span>
                        ...
                        <div class="row">
                            ...
                            <span class="col-auto">{{ $rupiah(getTotalAmount(item?.transportPrice)) }}</span>
                        </div>
                        <div class="row">
                            <span class="col">Total Harga</span>
                            <span class="col-auto">
                                {{ $rupiah(Math.round(getTotalAmount(item?.transportPrice) * item?.quantity)) }}
                            </span>
                        </div>
                        <div class="row">
                            <div class="col">
                                <CalenderMultiWrapper>

                                    <CalenderCheckIn slot="calender1" @onBubbleEventUpdate="onBubbleEventUpdate($event, index)" @onBubbleEvent="records = []; records = $event"
                                        :selectedData="item"
                                        :id="item?.id"
                                        :limit="limit"
                                        :page="page"
                                        ...
                                        :perPage="perPage"
                                        :currentPage="currentPage"
                                        :date_checkin="item?.dateCheckin" />

                                    <CalenderBooked slot="calender2" :selectedData="item" />
                    ...
                    >
                        <vs-td>
                            <vs-button @click="$refs.SharedBrowserModal.onCall({
                              ...
                              url: '/api/typehead/transport/dialog_cart_price' })">
                      ...
                      <vs-td
                        ...
                        "
                        :style="dataRow.field == 'ui_date_range' ? 'min-width: 500px;' : ''"
                      >
                        <template v-if="dataRow.browse == 1">
                          <img
                          ...
                          <span v-else-if="dataRow.type == 'relation'">{{
                            ...
                            <div :class="[ dataRow.field == 'ui_date_range' ? 'row' : '']" v-else>
                                ...
                                <span v-if="dataRow.field == 'name'">
                                    {{ record.transportPrice?.name }}
                                </span>
                                <span v-else-if="dataRow.field == 'get_price'">
                                    {{ $rupiah(record.transportPrice?.generalPrice) }}
                                </span>
                                <span v-else-if="dataRow.field == 'get_discount'">
                                    {{ record.transportPrice?.discountPrice }}%
                                </span>
                                <span v-else-if="dataRow.field == 'get_cashback'">
                                    {{ $rupiah(record.transportPrice?.cashbackPrice) }}
                                </span>
                                <span v-else-if="dataRow.field == 'get_total_amount'">
                                    {{ $rupiah(getTotalAmount(record?.transportPrice)) }}
                                </span>
                                <span v-else-if="dataRow.field == 'get_final_amount'">
                                    {{ $rupiah(Math.round(getTotalAmount(record?.transportPrice) * record.quantity)) }}
                                </span>
...
import CalenderMultiWrapper from "./CalenderMultiWrapper.vue"
...
export default {
  components: { downloadExcel, StackModal, CounterDate, CalenderBooked, CalenderCheckIn, CalenderMultiWrapper },
  ...
  watch: {
    ...
    selected(val) {
        ...
        let total = 0
        val.forEach(element => {
            total += Number(this.getTotalAmount(element?.transportPrice) * element.quantity)
        });
  ...
  methods: {
    ...
    async onPopupBooking() {
        ...
        this.$openLoader();
        await axios.post('/api/typehead/transport/get_prices_booking', bodyFormData, {
            headers: {
    ...
    async onInvoice() {
        ...
        this.$openLoader();
        await axios.post('/trevolia-api/v1/entities/transport-carts/add', bodyFormData, {
            headers: {
                Authorization: `Bearer ${localStorage.getItem('token')}`
            }
        })
    ...
    onUpdateQuantity: _.debounce(async function(value) {
        ...
        bodyFormData.append('dateCheckIn', value.dateCheckIn);
        bodyFormData.append('dateCheckOut', value.dateCheckOut);
