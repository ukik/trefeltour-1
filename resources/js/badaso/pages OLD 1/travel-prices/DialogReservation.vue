...
<template>
    <div class="mb-2 mt-3 p-0 col ml-3 pr-2 row">
        ...
        <label class="badaso-text__label col-12 p-1">Pilih Reservasi</label>
        ...
        <vue-typeahead-bootstrap disabled ref="typeahead" class="col p-0" :class="[ $route?.name == 'CrudGeneratedEdit' ? 'mr-4' : '']"  v-model="query" :ieCloseFix="false" :data="users"
            :serializer="item => { return `Reservasi UUID (${item.uuid})` }"
            @hit="selecteduser = $event" placeholder="Pilih Reservasi" @input="lookupUser" required>
            ...
            <slot name="modal-header">
                <div class="modal-header">
                    <h3 class="modal-title">
                        {{  type == 'detail' ? 'Detail' : 'Pilih' }} Reservasi
                    </h3>
...
export default {
    ...
    async mounted() { this.$openLoader();
        ...
        if(this.$route.params?.id) {
            axios
                .get(`/api/typehead/travel/dialog_prices_travel_reservations?id=` + this.$route.params?.id, {
                    headers: {
                ...
                .then(response => {
                    ...
                    this.$refs.typeahead.inputValue = `Reservasi UUID (${item.uuid})` //`Profile UUID (${item.uuid}) - Nama (${item?.badasoUser.name}) - Username (${item?.badasoUser.username}) - Email (${item?.badasoUser.email}) - Telpon (${item?.badasoUser.phone})`
    ...
    methods: {
        onBubbleEvent(response) {
            ...
            const item = response
            this.$refs.typeahead.inputValue = `Reservasi UUID (${item.uuid})`
        ...
        lookupUser: debounce(function () {
            ...
            axios
                .get('/api/typehead/travel/dialog_prices_travel_reservations?keyword=' + this.query, {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem('token')}`
                    }
                })
