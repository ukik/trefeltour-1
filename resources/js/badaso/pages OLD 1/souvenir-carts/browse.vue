                            ...
                            <vs-row vs-w="12" style="width:100px;">
                                <vs-col class="p-0" vs-type="flex" vs-justify="center" vs-align="center" vs-w="6">
                                    <vs-button @click="$refs.SharedBrowserModal.onCall({
                                        ...
                                        url: '/api/typehead/souvenir/dialog_cart_price' })">
                          ...
                          <span v-else-if="dataRow.type == 'relation'">{{
                            ...
                            <div :class="[ dataRow.field == 'quantity' ? 'row' : '']" v-else>
                                ...
                                <div class="full-width" v-else-if="dataRow.field == 'quantity'">
                                    <Counter :selectedId="record.id" @onBubbleEvent="onUpdateQuantity" :set_quantity="record.quantity" :stock="record?.souvenirPrice?.stock" />
                                </div>
                                <span v-else>
                                {{
                                    record[
...
export default {
  ...
  methods: {
    ...
    onUpdateQuantity: _.debounce(async function(value) {
        ...
        this.$openLoader();
        await axios.post('/api/typehead/souvenir/update_to_cart', bodyFormData, {
            headers: {
                Authorization: `Bearer ${localStorage.getItem('token')}`
            }
        })
