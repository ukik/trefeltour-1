    ...
    <div class="mb-2 mt-3 p-0 col ml-3 pr-2 row">
        ...
        <stack-modal
            ...
            <shared-read-user :response="{
                data: selecteduser
            }" v-if="type=='detail'" slug="transport-payments"></shared-read-user>

            <shared-table-modal-payment-validation v-if="type=='select'" @onBubbleEvent="onBubbleEvent" slug="transport-payments" />
...
export default {
    ...
    async mounted() { this.$openLoader();
        ...
        if(this.$route.params?.id) {
            axios
                .get(`/api/typehead/transport/dialog_booking_transport_payments_validations?id=` + this.$route.params?.id, {
                    headers: {
    ...
    methods: {
        ...
        lookupUser: debounce(function () {
            ...
            axios
                .get('/api/typehead/transport/dialog_booking_transport_payments_validations?keyword=' + this.query, {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem('token')}`
                    }
                })
