...
<template>
    <div class="mb-2 mt-3 p-0 col ml-3 pr-2 row">
        ...
        <label class="badaso-text__label col-12 p-1">Pilih Rental</label>
        ...
        <vue-typeahead-bootstrap disabled ref="typeahead" class="col p-0" :class="[ $route?.name == 'CrudGeneratedEdit' ? 'mr-4' : '']"  v-model="query" :ieCloseFix="false" :data="users"
            :serializer="item => { return `UUID (${item.uuid}) Rental (${item.name})` }"
            @hit="selecteduser = $event" placeholder="Pilih Rental" @input="lookupUser" required>
        ...
        <stack-modal
            ...
            <shared-read-user :response="{
                data: selecteduser
            }" v-if="type=='detail'" slug="transport-rentals"></shared-read-user>

            <shared-table-modal v-if="type=='select'" @onBubbleEvent="onBubbleEvent" slug="transport-rentals" />
...
export default {
    ...
    async mounted() { this.$openLoader();
        ...
        if(this.$route.params?.id) {
            axios
                .get(`/api/typehead/transport/dialog_rental_transport_vehicles?id=` + this.$route.params?.id, {
                    headers: {
                ...
                .then(response => {
                    ...
                    this.$refs.typeahead.inputValue = `UUID (${item.uuid}) Rental (${item.name})`
    ...
    methods: {
        ...
        lookupUser: debounce(function () {
            ...
            axios
                .get('/api/typehead/transport/dialog_rental_transport_vehicles?keyword=' + this.query, {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem('token')}`
                    }
                })
