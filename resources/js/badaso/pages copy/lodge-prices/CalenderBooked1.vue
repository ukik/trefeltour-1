<template>
    <div>
        {{ attributes }}
        <!-- :min-date='new Date()' -->
    <DatePicker :min-date='getYesterday' :value="days"
...
export default {
    ...
    methods: {
        onDayClick(day) {
            console.log('methods', today_format, day.id)
            console.log('today < click', ((new Date(day.id)) < new Date(today_format)  ))

            if(((new Date(day.id)) < new Date(today_format)  )) return
            ...
            this.$emit('onBubbleEvent', this.combinedAttrs)
            // this.combinedAttrs.sort(function(a,b){
            //     // Turn your strings into dates, and then subtract them
            //     // to get a value that is either negative, positive, or zero.
            //     return new Date(b.dates) - new Date(a.dates);
