...
class TourPaymentsValidationsController extends Controller
{
    ...
    public function browse(Request $request)
    {
        try {
            ...
            $data = \TourPaymentsValidations::with([
                'badasoUser',
                'badasoUsers',
                'tourPayments',
                'tourPayment',
                'tourPayment.tourBooking',
            ...
            if(request()->search) {
                ...
                $columns = \Illuminate\Support\Facades\Schema::getColumnListing('tour_payments_validations');
                ...
                $data
                    ...
                    ->orWhereHas('tourPayment', $payment_id);
            ...
            if(isClientOnly()) {
                $data->whereHas('tourPayment',function($q){
    ...
    public function read(Request $request)
    {
        ...
        try {
            ...
            $data = \TourPaymentsValidations::query();
    ...
    public function edit(Request $request)
    {
        ...
        $check = TourPaymentsValidations::where('id', $value)->where('is_valid', 'true')->first();
        ...
        try {
            ...
            $table_entity = \TourPaymentsValidations::where('id', $request->data['id'])->first();
            $temp = \TourPayments::where('id', $request->data['payment_id'])->first();
            ...
            $data = [
                ...
                'code_table' => ('tour-payments-validations'),
            ...
            $validator = Validator::make(
                ...
                [
                    ...
                    'payment_id' => [
                        'required', \Illuminate\Validation\Rule::unique('tour_payments_validations')->ignore($table_entity->id)
                    ],
                ],
            );
            if ($validator->fails()) {
            ...
            \TourPaymentsValidations::where('id', $request->data['id'])->update($data);
            $updated['old_data'] = $table_entity;
            $updated['updated_data'] = \TourPaymentsValidations::where('id', $request->data['id'])->first();

            // NOTIFICATION
            $payload = TourPaymentsValidations::where('id', $request->data['id'])->first();
            if($payload['is_valid'] === 'true') {
                NotifyToAdmin(new NotifyClientToAdminNotification(Auth::user(), 'tour', 'tour-payments-validations', $payload, 'Pembayaran Valid'));
            }
    ...
    public function add(Request $request)
    {
        ...
        $unique = TourPaymentsValidations::where('payment_id', $request->data['payment_id'])
        ...
        try {
            ...
            $validator = Validator::make(
                ...
                [
                    '*' => 'required',
                    // susah karena pake softDelete, pakai cara manual saja
                    'payment_id' => 'unique:tour_payments_validations'
                ],
            );
            if ($validator->fails()) {
                $errors = json_decode($validator->errors(), True);
            ...
            $stored_data = \TourPaymentsValidations::insert($data);

            $temp->update(['is_selected' => 'true']);

            // Booking Condition
            if($req['is_valid'] === 'true') TourBookings::where('id', $temp->booking_id)->update(['condition' => 'lunas']);
            ...
            // NOTIFICATION
            $payload = TourPaymentsValidations::where('payment_id', $request->data['payment_id'])->first();
            if($payload['is_valid'] === 'true') {
                NotifyToAdmin(new NotifyClientToAdminNotification(Auth::user(), 'tour', 'tour-payments-validations', $payload, 'Pembayaran Valid'));
            } else {
                NotifyToAdmin(new NotifyClientToAdminNotification(Auth::user(), 'tour', 'tour-payments-validations', $payload, 'Pembayaran Waiting'));
            }
    ...
    public function deleteMultiple(Request $request)
    {
        ...
        try {
            ...
            // -------------------------------------------- //
            $filters = TourPaymentsValidations::whereIn('id', explode(",", request()['data'][0]['value']))->where('is_valid', 'false')->get();
            $temp = [];
