...
class CampusBookingController extends BadasoBaseController
{
    ...
    public function edit(Request $request)
    {
        ...
        try {
            ...
            if($request['data']['status'] == 'done') {
                DB::table('campus_rooms')->where('id', $request['data']['room_id'])
                ->increment('slot',1);
                // ->update([
                //     'slot' => 4, //DB::raw('slot - 1'),
                // ]);
            }
        ...
        try {
            ...
            $validator = Validator::make(
                ...
                [
                    ...
                    'status' => [ 'required' ],
                    // 'email' => [
                    //     'required','max:255', 'email', //'email:rfc,dns' //'email', \Illuminate\Validation\Rule::unique('users')->ignore($this->me)
                    // ],
                    // 'name' => [
                    //     'required','max:255', //\Illuminate\Validation\Rule::unique('users')->ignore($this->me)
                    // ],
                    // 'phone' => [
                    //     'required','max:30', //\Illuminate\Validation\Rule::unique('users')->ignore($this->me)
                    // ],
                    // 'subject' => [
                    //     'required','max:255', //\Illuminate\Validation\Rule::unique('users')->ignore($this->me)
                    // ],
                    // 'description' => [
                    //     'required', //\Illuminate\Validation\Rule::unique('users')->ignore($this->me)
                    // ],
                    // 'recaptcha' => 'recaptcha',
                ],
            );
            ...
            if ($validator->fails()) {
                return ApiResponse::failed($validator->errors());
            }

            // CHECK if campus_rooms > slot tersedia
            $room = DB::table('campus_rooms')->where('id', $request['data']['room_id'])->first();
            if($room->slot > 0) {
                DB::table('campus_rooms')->where('id', $request['data']['room_id'])
                ->decrement('slot',1);
                // ->update([
                ...
                return ApiResponse::failed("{$room->name} slot habis");
            }
