...
class BadasoAuthController extends Controller
{
    ...
    {
        $this->middleware(config('badaso.middleware.authenticate'), [
            'except' => [
                'secretLogin',
                'login',
                'register',
    ...
    public function init(Request $request)
    {
        try {
            ...
            //     ],
            // ])
            // ->header('Access-Control-Expose-Headers', 'cubix_csrf')
            // ->header('cubix_csrf', $csrf)
            // ->header('cubix_token_2', $token)
            // ->withCookie('cubix_csrf', $csrf, config('jwt.ttl'), '/')
            // ->withCookie('cubix_token_2', $token, config('jwt.ttl'), '/');
    ...
    protected function createNewToken($token, $user, $remember = false)
    {
        ...
        $obj->user = $user;
        $obj->roles = json_encode($user?->roles, true);
