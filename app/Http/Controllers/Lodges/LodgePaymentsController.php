...
class LodgePaymentsController extends Controller
{
    ...
    public function browse(Request $request)
    {
        try {
            ...
            if(request()->search) {
                ...
                $columns = \Illuminate\Support\Facades\Schema::getColumnListing('lodge_payments');
    ...
    public function add(Request $request)
    {
        ...
        try {
            ...
            // NOTIFICATION
            $payload = LodgePayments::where('booking_id', $request->data['booking_id'])->first();
            if($payload['is_valid'] === 'true') {
                NotifyToAdmin(new NotifyClientToAdminNotification(Auth::user(), 'lodge', 'lodge-payments', $payload, 'Booking Diproses'));
            }
