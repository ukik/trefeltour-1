...
class TourismPricesController extends Controller
{
    ...
    public function lagia_browse(Request $request)
    {
        try {
            ...
            if(request()->search) {
                ...
                $columns = \Illuminate\Support\Facades\Schema::getColumnListing('tourism_prices');
            ...
            $additional = NULL;
            if(request()->product) {
                $data = $data->where('venue_id',request()->product);
                $additional = TourismVenues::whereId(request()->product)->with(['ratingAvg'])->first();
            }
            // ================================================
    ...
    public function edit(Request $request)
    {
        ...
        try {
            ...
            $data = [
                ...
                'venue_id' => $table_entity->venue_id,
            ...
            \TourismPrices::where('id', $request->data['id'])->update($data);
    ...
    public function add(Request $request)
    {
        ...
        try {
            ...
            $req = request()['data'];
            $venue_id = TourismVenues::where('id', $req['venue_id'])->value('id');
            ...
            $stored_data = \TourismPrices::insert($data);
    ...
    public function delete(Request $request)
    {
        ...
        $check = TourismPrices::where('id', $value)->with(['tourismCart'])->first();
        if($check->tourismCart) return ApiResponse::failed("Tidak bisa dihapus, data ini digunakan");
    ...
    public function deleteMultiple(Request $request)
    {
        ...
        try {
            ...
            // -------------------------------------------- //
            $filters = TourismPrices::whereIn('id', explode(",",request()['data'][0]['value']))->with('tourismCart')->get();
            $temp = [];
            foreach ($filters as $value) {
                if($value->tourismCart == null) {
